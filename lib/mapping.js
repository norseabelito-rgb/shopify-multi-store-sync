...
// construiește payload-ul Shopify din product + psRow + store
...
// diffs între produsul existent și payload-ul nou
function diffProduct(existing, payload) {
  ...
  const eImgs = (existing.images || []).map((i) => i.src).filter(Boolean);
  const pImgs = (payload.images || []).map((i) => i.src).filter(Boolean);
  const sameImages =
    eImgs.length === pImgs.length &&
    eImgs.every((src, idx) => src === pImgs[idx]);
  if (!sameImages) changed.push('poze');
...
// logică pentru create / update / delete
