  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (isTransient && attempt < retries) {
        console.warn(`[db] Transient error (attempt ${attempt}/${retries}): ${err.message}`);
...
/**
 * Execute a slow query (like index creation) with extended timeout
 * Uses a dedicated connection to avoid affecting pool settings
 * @param {string} text - SQL query
 * @returns {Promise<object>} Query result
 */
async function slowQuery(text) {
  const p = getPool();
  const client = await p.connect();
  try {
    // Disable statement timeout for this connection only
...
async function healthCheck() {
  ...
  try {
    await query('SELECT 1', [], { retries: 1 });
    return {
      healthy: true,
...
async function initDb() {
  ...
  // Tracks daily product sales per SKU per store for top sellers analytics
  // Only includes SKUs that exist in products_master (our products)
  // Excludes cancelled/voided orders
  await query(`
    CREATE TABLE IF NOT EXISTS product_sales_daily_agg (
      agg_date DATE NOT NULL,
      store_id TEXT NOT NULL,
      sku TEXT NOT NULL,
      ...
      orders_count INT NOT NULL DEFAULT 0,
      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      PRIMARY KEY (agg_date, store_id, sku)
    );
  `);

  await query(`CREATE INDEX IF NOT EXISTS idx_product_sales_daily_agg_store_date ON product_sales_daily_agg (store_id, agg_date DESC);`);
  await query(`CREATE INDEX IF NOT EXISTS idx_product_sales_daily_agg_sku ON product_sales_daily_agg (sku);`);
  await query(`CREATE INDEX IF NOT EXISTS idx_product_sales_daily_agg_store_sku_date ON product_sales_daily_agg (store_id, sku, agg_date DESC);`);
  await query(`CREATE INDEX IF NOT EXISTS idx_product_sales_daily_agg_date ON product_sales_daily_agg (agg_date DESC);`);

  console.log('[db] product_sales_daily_agg table initialized (product sales aggregation)');
  ...
  // Add image columns if they don't exist (migration for existing tables)
  await query(`ALTER TABLE products_store_sync ADD COLUMN IF NOT EXISTS images_synced_at TIMESTAMPTZ;`);
  await query(`ALTER TABLE products_store_sync ADD COLUMN IF NOT EXISTS images_count INT DEFAULT 0;`);
  await query(`ALTER TABLE products_store_sync ADD COLUMN IF NOT EXISTS images_fingerprint TEXT;`);
  ...
  // Products images cache - stores Drive image URLs per SKU (DB-first approach)
  // This avoids slow Drive API calls during browsing
  await query(`
  ...
  await query(`CREATE INDEX IF NOT EXISTS idx_products_images_sku ON products_images (sku);`);
  await query(`CREATE INDEX IF NOT EXISTS idx_products_images_fetched ON products_images (fetched_at);`);

  console.log('[db] products_images table initialized (Drive image cache)');

  // Bulk jobs tracking - for import, push, and other async operations with progress
  await query(`
  ...
  // Job type values: 'import', 'push', 'push_all', 'push_selected'
  // Job status values: 'pending', 'running', 'completed', 'failed', 'cancelled'
  await query(`CREATE INDEX IF NOT EXISTS idx_bulk_jobs_type ON bulk_jobs (type);`);
  await query(`CREATE INDEX IF NOT EXISTS idx_bulk_jobs_status ON bulk_jobs (status);`);
  await query(`CREATE INDEX IF NOT EXISTS idx_bulk_jobs_store ON bulk_jobs (store_id) WHERE store_id IS NOT NULL;`);
  await query(`CREATE INDEX IF NOT EXISTS idx_bulk_jobs_created ON bulk_jobs (created_at DESC);`);

  console.log('[db] bulk_jobs table initialized');

  // ==================== MIGRATIONS TRACKING (Item 17) ====================
  await query(`
  ...
  `);
  console.log('[db] schema_migrations table initialized');

  // ==================== ADDITIONAL INDEXES (Items 8, 18) ====================

  // Composite index for products_store_sync queries
  await query(`CREATE INDEX IF NOT EXISTS idx_products_store_sync_store_status
    ON products_store_sync (store_id, status);`);

  // Index for orders by financial_status (common filter)
  await query(`CREATE INDEX IF NOT EXISTS idx_orders_financial_status
    ON orders_index (store_id, financial_status);`);

  // GIN index for orders_detail JSONB queries (can be slow on large tables)
  console.log('[db] Creating GIN index on orders_detail (this may take a while)...');
  await slowQuery(`CREATE INDEX IF NOT EXISTS idx_orders_detail_raw_json_gin
    ON orders_detail USING GIN (raw_json jsonb_path_ops);`);

  // GIN index for customers_detail JSONB queries (can be slow on large tables)
  console.log('[db] Creating GIN index on customers_detail (this may take a while)...');
  await slowQuery(`CREATE INDEX IF NOT EXISTS idx_customers_detail_raw_json_gin
    ON customers_detail USING GIN (raw_json jsonb_path_ops);`);

  console.log('[db] Additional indexes created');

  // ==================== SOFT DELETE (Item 19) ====================
  await query(`ALTER TABLE products_master ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ;`);
  await query(`CREATE INDEX IF NOT EXISTS idx_products_master_deleted
    ON products_master (deleted_at) WHERE deleted_at IS NOT NULL;`);
  console.log('[db] Soft delete column added to products_master');
  ...
  `);
  await query(`CREATE INDEX IF NOT EXISTS idx_audit_log_entity
    ON audit_log (entity_type, entity_id);`);
  await query(`CREATE INDEX IF NOT EXISTS idx_audit_log_created
    ON audit_log (created_at DESC);`);
  await query(`CREATE INDEX IF NOT EXISTS idx_audit_log_actor
    ON audit_log (actor) WHERE actor IS NOT NULL;`);
  console.log('[db] audit_log table initialized');

  // ==================== STORE TOKENS TABLE (Item 16) ====================
  // Stores encrypted tokens instead of environment variables
  await query(`
  ...
  `);
  console.log('[db] store_credentials table initialized');

  // ==================== WEBHOOKS TABLE (Item 36) ====================
  await query(`
  ...
  `);
  await query(`CREATE INDEX IF NOT EXISTS idx_webhooks_store
    ON shopify_webhooks (store_id);`);
  console.log('[db] shopify_webhooks table initialized');

  // ==================== RECORD MIGRATION VERSION ====================
  await query(`
    INSERT INTO schema_migrations (version, description)
    VALUES ('2025_12_18_001', 'Add migrations tracking, indexes, soft delete, audit log, webhooks')
